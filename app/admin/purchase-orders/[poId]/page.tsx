...
export default function POPage({ params }: { params: Promise<{ poId: string }> }) {
  ...
  async function sendByEmail() {
    ...
    if (po.ordered_at) lines.push(`■ 発注日: ${parseDbDate(po.ordered_at).toLocaleDateString("ja-JP")}`)
    lines.push(`■ 納期希望: ${po.expected_at ? parseDbDate(po.expected_at).toLocaleDateString("ja-JP") : "最短"}`)
    lines.push("")
    lines.push("【明細】")
    items.forEach((i, idx) => {
    ...
    <div className="space-y-3">
      ...
      <main className="bg-white rounded-lg p-8 max-w-3xl mx-auto print-area" style={{ border: "1px solid #e8eaed" }}>
        ...
        <table style={{ width: "100%", marginTop: 18, borderCollapse: "collapse", fontSize: 12 }}>
          <tbody>
            <tr>
              <td style={td}>発注日</td><td style={td2}>{po.ordered_at ? parseDbDate(po.ordered_at).toLocaleDateString("ja-JP") : "—"}</td>
              <td style={td}>納期希望</td><td style={td2}>{po.expected_at ? parseDbDate(po.expected_at).toLocaleDateString("ja-JP") : "最短"}</td>
            </tr>
            <tr>
              <td style={td}>状態</td><td style={td2}>{po.status}</td>
              <td style={td}>送付方法</td><td style={td2}>
                {po.sent_method ? `${po.sent_method}${po.sent_at ? `（${parseDbDate(po.sent_at).toLocaleString("ja-JP", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}）` : ""}` : "— 未送付"}
              </td>
            </tr>
          </tbody>
        </table>
